 ...
 */
export function assignTiers(installed, usage, report) {
    ...
    return installed.map((plugin) => {
        ...
        if (use.calls === 0) {
            ...
            if (stale)
                reasons.push('not updated for over a year');
            if (clustered.has(plugin.name))
                reasons.push('redundant with another plugin');
 ...
 */
export function renderMermaidGraph(installedNames, matrix, threshold, plugins = []) {
    ...
    lines.push('  classDef installed fill:#183a52,stroke:#57c7d4,color:#f4fbff,stroke-width:2px', '  classDef candidate fill:#202735,stroke:#8492a6,color:#e7edf5,stroke-dasharray:5 4');
    for (const name of nodes)
        lines.push(`  class ${ids.get(name)} ${installed.has(name) ? 'installed' : 'candidate'}`);
...
function functionSummaryZh(plugin) {
    if (plugin === undefined)
        return '功能说明暂缺';
    ...
    if (plugin.description.length > 0)
        return `功能：${plugin.description}`;
    return '功能说明暂缺';
}
/** Explain the weighted dimensions that make one edge strong. */
function similarityReasons(cell) {
    const reasons = [];
    if (cell.textCosine >= 0.35)
        reasons.push('说明相近');
    if (cell.capabilityJaccard > 0)
        reasons.push(`功能重叠${Math.round(cell.capabilityJaccard * 100)}%`);
    if (cell.dependencyJaccard > 0)
        reasons.push(`依赖重叠${Math.round(cell.dependencyJaccard * 100)}%`);
    return reasons.length > 0 ? reasons.join('、') : '综合相似';
}
/** Render a Chinese, scan-friendly explanation for the strongest relations. */
export function renderSimilarityDetails(matrix, plugins, threshold) {
    const metadata = new Map(plugins.map(plugin => [plugin.name, plugin]));
    const pairs = [...matrix].filter(cell => cell.overall > threshold).sort((a, b) => b.overall - a.overall).slice(0, MAX_GRAPH_EDGES);
    if (pairs.length === 0)
        return '当前没有超过阈值的插件关系。';
 ...
 */
export function renderClusterTree(clusters) {
    if (clusters.length === 0)
        return 'No redundancy clusters above the threshold.';
    return clusters.map(cluster => `- cluster (${(cluster.cohesion * 100).toFixed(0)}% cohesion)\n${cluster.members.map(member => `  - ${member}`).join('\n')}`).join('\n');
}
/**
 ...
 */
export function renderLandscape(tiers, report, threshold, installedNames, plugins, sessionsScanned) {
