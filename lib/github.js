...
export class CommunitySource {
     ...
     * @param signal - cancellation signal from the tool execution.
     */
    async inspectInstallRefs(refs, signal) {
        return Promise.all(refs.map(async (ref) => {
            const repo = normalizeGitHubRepo(ref);
            if (repo === undefined)
                return { ref, error: 'only GitHub repository refs or URLs can be inspected', errorKind: 'unsupported' };
            try {
                const metadata = await fetchJson(this.deps, `https://api.github.com/repos/${repo}`, signal);
                ...
                if (!(packageResult.status >= 200 && packageResult.status < 300)) {
                    const errorKind = packageResult.status === 403 || packageResult.status === 429 ? 'rate-limit' : 'http';
                    return { ref, repo, error: `package.json returned HTTP ${packageResult.status}`, errorKind };
                }
                const packageJson = JSON.parse(packageResult.text);
                ...
                if (patchPath.startsWith('/') || patchPath.split('/').includes('..'))
                    throw new Error(`unsafe bundle patch path: ${patchPath}`);
                ...
                return { ref, repo, error: message, errorKind: /\b(?:403|429)\b|rate.?limit/i.test(message) ? 'rate-limit' : 'network' };
            }
        }));
    }
    /**
...
function normalizeGitHubRepo(ref) {
