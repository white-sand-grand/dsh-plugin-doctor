 ...
 */
function tfidfCosine(docs, corpus) {
    const weight = (doc, term) => (doc.get(term) ?? 0) * idf(term, corpus);
    let dot = 0;
    let normA = 0;
    let normB = 0;
 ...
 */
export function similarityMatrix(plugins) {
 ...
 */
export function redundancyClusters(plugins, matrix, threshold) {
    const edges = matrix.filter(cell => cell.overall > threshold).sort((x, y) => y.overall - x.overall);
    const clustered = new Set();
    const clusters = [];
    for (const edge of edges) {
        if (clustered.has(edge.a) || clustered.has(edge.b))
            continue;
        const members = new Set([edge.a, edge.b]);
        let cohesion = edge.overall;
        for (const plugin of plugins) {
            if (clustered.has(plugin.name) || members.has(plugin.name))
                continue;
            const linked = edges.some(link => (link.a === plugin.name && members.has(link.b)) || (link.b === plugin.name && members.has(link.a)));
            if (linked)
                members.add(plugin.name);
        }
 ...
 * @param plugins - the compared set.
 */
export function irreplaceabilityScores(plugins) {
    ...
    return plugins.map(plugin => {
        ...
        if (unique.length > 0)
            reasons.push(`unique capabilities: ${unique.join(', ')}`);
        const starScore = maxStars === 0 ? 0.5 : plugin.stars / maxStars;
        if (plugin.stars > 0)
            reasons.push(`${plugin.stars} stars`);
        ...
        if (!Number.isNaN(ageDays) && ageDays <= 90)
            reasons.push(`updated ${Math.round(ageDays)} days ago`);
 ...
 */
export function analyze(plugins, threshold) {
