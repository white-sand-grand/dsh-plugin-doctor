...
export async function searchOfficialIssues(deps, refs, intent, signal) {
    const repositories = [...new Set(refs.map(normalizeRepo).filter((repo) => repo !== undefined))];
    const terms = [...new Set([
            ...tokens(intent),
    ...
    for (const repo of repositories) {
        try {
            ...
            for (const item of payload) {
                if (item.pull_request !== undefined || typeof item.title !== 'string' || typeof item.html_url !== 'string')
                    continue;
