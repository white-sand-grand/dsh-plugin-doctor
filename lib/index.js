...
import { analyzeInstallConflicts } from "./install-check.js";
import { searchOfficialIssues } from "./issues.js";
import { readInventory, readPeerDependencies, readPluginRows, readRecommendRows, toPluginRows, resolveDshHome } from "./inventory.js";
import { assignTiers, renderLandscape } from "./landscape.js";
import { OfficialReleaseSource, appendBehindContent, assembleSyncStatus, detectLocalVersion, renderSyncAdvisory } from "./official-sync.js";
 ...
 */
function runDshPlugin(profile, action) {
    ...
    if (result.error !== undefined) {
        ...
        if (code === 'ENOENT')
            return { ok: false, detail: 'dsh not found on the server PATH' };
        if (code === 'ETIMEDOUT')
            return { ok: false, detail: 'timed out after 180s' };
...
function probeDshVersion() {
    ...
    if (result.error !== undefined) {
        const code = result.error.code;
        if (code === 'ENOENT')
            return { ok: false, detail: 'dsh not found on PATH' };
        if (code === 'ETIMEDOUT')
    ...
    if (result.status !== 0 || output.length === 0)
        return { ok: false, detail: `exit ${result.status}` };
...
export function apply(ctx, config) {
    const declinedIssueReview = new WeakSet();
    ...
    const issueReview = async (intent, refs, exec) => {
        const session = typeof exec.agent === 'object' && exec.agent !== null ? exec.agent : noAgentSession;
        if (declinedIssueReview.has(session))
            return { issues: [] };
        const ask = askChoiceFactory(ctx.get('userQuestions'));
        const choice = await ask('Before suggesting or installing plugins, should I check their official GitHub repositories for open issues that may explain known failures or compatibility risks?', 'Official issue safety check', [
        ...
        if (choice !== 'check')
            return { issues: [], note: 'Official issue check was not completed; no issue lookup was performed.' };
        await refreshToken();
        return searchOfficialIssues({ web: ctx.get('web'), get token() { return token; } }, refs, intent, exec.signal);
    };
    const syncSource = new OfficialReleaseSource({ web: ctx.get('web'), get token() { return token; } });
    ...
    const runOfficialSync = async (profile, signal) => {
        await refreshToken();
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const output = {
                ...
                ...(result.degraded === undefined && issueReviewResult.note === undefined ? {} : { degraded: [result.degraded, issueReviewResult.note].filter((note) => note !== undefined).join(' ') }),
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const decision = await recommend(args.intent, search, await readRecommendRows(profile, inventory.names), current().similarityThreshold ?? DEFAULT_THRESHOLD, profile, hooks, exec.signal);
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Preflight two or more GitHub DSH plugin repositories before installing them together. '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Compare the locally installed DSH version with the latest official deepseek-harness GitHub '
...
function renderOfficialIssueNotice(issues, note) {
    if (issues.length === 0 && note === undefined)
        return '';
