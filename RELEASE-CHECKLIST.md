...
- [ ] No secret is hardcoded: token only via `githubTokenEnv` credential reference (default `DSH_PLUGIN_RECOMMENDER_GITHUB_TOKEN`) or the `role('secret')` literal field; grep the diff for token-like strings.
