...
插件会先询问是否进行官方 Issue 检查。实际排查时，官方仓库曾发现与“右侧文件面板卡顿、历史加载失败”高度一致的未关闭 Issue：根因并不是远程 Web 插件，而是某个文件面板插件的递归监视行为。报告会显示模糊化后的插件名，例如 `@example-org/ui-panel-plugin`，并附上官方 [Issue #119](https://github.com/zhu1090093659/dsh-web-ui/issues/119) 供用户核对。用户在本会话明确拒绝后，后续安装请求不再重复提醒。
