...
> ✅ **当前状态：完整可用。** 已在 DSH v0.1.0-rc.6 上通过全部验证：构建、54 个测试、真实工具注册与调用、以及 Web UI 中的真实对话测试。装上即用。
>
...
| "这个和我装的那个功能重复吗" | 摆出重叠度事实后**先问你怎么处理**：留 A 删 B（附命令；开启 `allowExecuteActions` 后确认即真执行）/ 把重复的合并成一个新的自研插件（生成整合设计书）/ 先不动 |
| "我想要一个 XXX，但找不到" | 列出功能相近的竞品和它们各自缺什么，**问过你要不要自己开发**、你确认后才生成完整的开发规格书 |
| "我装的插件哪些根本没用过" | 扫描本地会话日志统计每个插件工具的真实调用量，零调用的给出卸载命令（纯本地，不出网） |
| "之前装的插件有什么联系" | 直接展示精简的相似度关系图：节点附中文功能解释，连线标明相似度以及说明、功能或依赖重叠在哪里 |
...
- **`plugin_recommend`**：综合以上两步给出最终建议。凡是涉及"生成设计书"或"卸载插件"的结论，都会先弹出选项让你确认（Web UI 里是选择题卡片）；不方便交互的环境自动退回直接给建议的模式。默认只给出命令；把 `allowExecuteActions` 打开后，你在弹窗里确认的"留 A 删 B"会被真正执行（装保留者、卸冗余者，逐条回报结果），非交互路径永不执行。
- **`plugin_install_guard`**：当你一次给出多个仓库地址、准备一起安装时，先检查各仓库的 `package.json`、工具名、Cordis patch 行和 peer 依赖。发现重复注册或不兼容主版本会明确报告 **INSTALL BLOCKED**；它只读检查，永远不会执行安装。
...
- **`plugin_landscape`**：插件全景图——当你问“之前装的插件有什么联系”“哪些插件相似”时直接展示。关系图使用区分已安装项与社区候选项的现代配色；每个节点附中文功能解释，每条边同时说明总相似度及其来源（说明文本、功能标签或依赖重叠），图后还有逐对中文解释。四档分层仍保留：`core`（重度使用或“在用且难替代”）、`active`（有一定使用）、`idle`（声明了工具但零调用）、`review`（零调用且更新超一年或与其他插件冗余）。带一个可选意图参数时，社区匹配项会以虚线节点加入图谱一并对比。
