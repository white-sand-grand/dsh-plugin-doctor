...
import { resolveDshHome } from './inventory.ts'
...
// ---------------------------------------------------------------------------
// Version parsing and comparison (prerelease-aware, no external semver)
// ---------------------------------------------------------------------------
 ...
 */
export function parseVersion(raw: string | undefined | null): ParsedVersion | undefined {
...
export function compareVersions(a: ParsedVersion, b: ParsedVersion): -1 | 0 | 1 {
  for (const [x, y] of [[a.major, b.major], [a.minor, b.minor], [a.patch, b.patch]] as const) {
...
function comparePrereleaseIdentifier(a: string, b: string): -1 | 0 | 1 {
  ...
  if (numA !== undefined && numB !== undefined) return numA < numB ? -1 : numA > numB ? 1 : 0
 ...
 */
export function rangeExcludes(range: string, candidate: ParsedVersion): boolean | undefined {
  const trimmed = range.trim()
  if (trimmed.length === 0 || trimmed === '*' || trimmed === 'x') return false
  ...
  for (const clauseText of trimmed.split('||')) {
    ...
    for (const token of tokens) {
      ...
      if (!satisfied) clauseSatisfied = false
    }
...
function tokenSatisfies(token: string, candidate: ParsedVersion): boolean | undefined {
  ...
  if (op === '^' || op === '~') {
    ...
    if (compareVersions(candidate, bound) < 0) return false
    if (op === '~') return candidate.major === bound.major && candidate.minor === bound.minor
    // Caret fixes the leftmost non-zero component (semver's ^0.x rules).
    if (bound.major > 0) return candidate.major === bound.major
    if (bound.minor > 0) return candidate.minor === bound.minor
  ...
  if (parts.some(part => WILDCARD_PARTS.has(part))) {
    ...
    if (!normalized.every(part => /^\d+$/.test(part))) return undefined
...
// ---------------------------------------------------------------------------
// Latest-release fetch with TTL cache and stale-cache rescue
// ---------------------------------------------------------------------------
 ...
 */
export function pickLatestRelease(payload: unknown): { release?: ReleaseInfo; cause?: string } {
  const cause = 'no published release found in the official repository'
  if (!Array.isArray(payload)) return { cause }
  for (const item of payload) {
    if (item === null || typeof item !== 'object') continue
    const entry = item as ReleaseEntry
    if (typeof entry.tag_name !== 'string' || entry.draft === true) continue
    return {
      release: {
        tag: entry.tag_name,
...
export class OfficialReleaseSource {
  private cache?: { fetchedAt: number; release: ReleaseInfo }

  /**
   * @param deps - ambient web capability / token holders.
   * @param ttlMinutes - cache lifetime in minutes.
   */
  constructor(private readonly deps: HttpDeps, private readonly ttlMinutes: number = RELEASES_TTL_MINUTES) {}

  /**
   * Fetch (or serve cached) latest official release data. Never throws.
   * @param signal - cancellation signal from the tool execution.
   */
  async latest(signal?: AbortSignal): Promise<{ release?: ReleaseInfo; degraded?: string }> {
    ...
    try {
      ...
      if (picked.release === undefined) return this.fail(picked.cause!)
  ...
  private fail(cause: string): { release?: ReleaseInfo; degraded?: string } {
    ...
    if (this.cache !== undefined) return { release: this.cache.release, degraded: `${note}; serving cached release data` }
    return { degraded: note }
  }
}

// ---------------------------------------------------------------------------
// Local DSH version detection (CLI probe with manifest fallback)
// ---------------------------------------------------------------------------
...
export type DshVersionProbe = () => { readonly ok: boolean; readonly output?: string; readonly detail?: string }
...
export async function detectLocalVersion(
  profile: string,
  probe: DshVersionProbe | undefined,
  env: Record<string, string | undefined> = process.env,
): Promise<LocalVersion> {
  const cliResult = probe === undefined ? undefined : cliVersion(probe)
  if (cliResult?.version !== undefined) return cliResult
  const manifestResult = await manifestVersion(profile, env)
  if (manifestResult.version !== undefined) return manifestResult
  const causes = [cliResult?.note, manifestResult.note].filter((note): note is string => note !== undefined)
  return { note: `local DSH version unknown${causes.length > 0 ? ` (${causes.join('; ')})` : ''}` }
}

function cliVersion(probe: DshVersionProbe): LocalVersion {
  const result = probe()
  if (!result.ok) return { note: `dsh --version could not be read (${result.detail ?? 'no output'})` }
  const version = parseVersion(result.output)
  if (version === undefined) return { note: `dsh --version could not be read (unrecognized output '${(result.output ?? '').slice(0, 40)}')` }
  return { raw: result.output, version, source: 'cli' }
}

async function manifestVersion(profile: string, env: Record<string, string | undefined>): Promise<LocalVersion> {
  ...
  try {
    ...
    if (typeof manifest.version === 'string') {
      ...
      if (version !== undefined) return { raw: manifest.version, version, source: 'manifest' }
    }
  } catch {
...
// ---------------------------------------------------------------------------
// Findings: what the new release duplicates or conflicts with
// ---------------------------------------------------------------------------
 ...
 */
export function analyzePeerConflicts(peers: readonly PeerDependencyRow[], release: ReleaseInfo): SyncFinding[] {
  const findings: SyncFinding[] = []
  if (release.version === undefined) return findings
  let skipped = 0
  for (const row of peers) {
    if (row.peer !== '@deepseek-ai/dsh' && !row.peer.startsWith('@deepseek-ai/dsh-')) continue
 ...
 */
export function analyzeToolTakeovers(toolOwners: ReadonlyMap<string, string>, release: ReleaseInfo): SyncFinding[] {
  const findings: SyncFinding[] = []
  if (release.body.length === 0) return findings
  for (const [tool, owner] of toolOwners) {
    if (tool.length < 3) continue
    if (!new RegExp(`(?<![A-Za-z0-9_])${escapeRegExp(tool)}(?![A-Za-z0-9_])`).test(release.body)) continue
    findings.push({
      kind: 'tool-takeover',
      severity: 'warning',
      refs: [owner],
 ...
 * @param release - the official release whose notes are scanned.
 */
export function analyzeCapabilityOverlaps(installed: readonly CommunityPlugin[], release: ReleaseInfo): SyncFinding[] {
  const bodyTokens = new Set(noteTokens(release.body))
  const qualified: { plugin: CommunityPlugin; shared: string[] }[] = []
  for (const plugin of installed) {
    ...
    if (shared.length >= 2 || shared.some(token => token.length >= 5)) qualified.push({ plugin, shared })
  }
...
function noteTokens(input: string): string[] {
...
const SOURCE_LABELS: Record<LocalVersionSource, string> = {
  cli: 'the `dsh --version` CLI',
  manifest: 'the installed package manifest',
}

function originLabel(raw: string | undefined, source: LocalVersionSource | undefined): string {
 ...
 */
export function assembleSyncStatus(local: LocalVersion, fetched: { release?: ReleaseInfo; degraded?: string }): OfficialSyncResult {
  if (fetched.release === undefined) {
    const causes = [local.note, fetched.degraded].filter((cause): cause is string => cause !== undefined)
    return {
      status: 'unknown',
      findings: [],
 ...
 */
export function appendBehindContent(result: OfficialSyncResult, input: BehindInput): OfficialSyncResult {
  ...
  if (input.inventoryNote !== undefined) lines.push('', `> Local inventory: ${input.inventoryNote}`)
 ...
 */
export function renderSyncAdvisory(sync: OfficialSyncResult | undefined): string {
  if (sync === undefined || sync.status !== 'behind') return ''
