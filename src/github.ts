...
/** One cached listing of the whole topic, plus when it was fetched. */
 ...
 */
const REGISTRY_SNAPSHOT: readonly CommunityPlugin[] = [
  {
...
/** Shape of one GitHub repository-search item this module reads. */
 ...
 */
function parseReadmeMetadata(excerpt: string): { capabilities: string[]; dependencies: string[] } {
...
export class CommunitySource {
   ...
   */
  async search(intent: string, filters: SearchFilters, signal: AbortSignal | undefined): Promise<SearchResult> {
  ...
  private async listing(signal: AbortSignal | undefined): Promise<{ plugins: readonly CommunityPlugin[]; degraded?: string }> {
    ...
    if (fresh) return { plugins: this.cache!.plugins }
    try {
   ...
   */
  private async fetchTopic(signal: AbortSignal | undefined): Promise<CommunityPlugin[]> {
    const url = `https://api.github.com/search/repositories?q=topic:${DSH_PLUGIN_TOPIC}&sort=stars&order=desc&per_page=30`
    const payload = await fetchJson(this.deps, url, signal) as { items?: GitHubRepo[] }
  ...
  private async readmeExcerpt(repo: string, signal: AbortSignal | undefined): Promise<string> {
    try {
      ...
      if (status >= 200 && status < 300) return text.slice(0, README_EXCERPT_CHARS)
    } catch {
 ...
 */
function overlapScore(keywords: readonly string[], plugin: CommunityPlugin): number {
