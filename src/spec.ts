 ...
 */
export function renderPluginSpec(intent: string, comparedAgainst: readonly CommunityPlugin[]): string {
 ...
 */
function suggestIntegrationName(intent: string, members: readonly CommunityPlugin[]): string {
 ...
 */
export function renderIntegrationSpec(intent: string, members: readonly CommunityPlugin[]): string {
  ...
  const memberRows = members.map(member =>
    `- \`${member.name}\` — ⭐ ${member.stars} · updated ${member.updatedAt || 'unknown'} · unique: ${
      member.capabilities.filter(capability => !members.some(other => other !== member && other.capabilities.includes(capability))).join(', ') || '(none)'
    }`)
  return [
    ...
    '',
    ...members.map(member => `- vs \`${member.name}\`: also covers ${
      allCapabilities.filter(capability => !member.capabilities.includes(capability)).join(', ') || 'the same scope, but maintained as one plugin'
    }`),
    '',
