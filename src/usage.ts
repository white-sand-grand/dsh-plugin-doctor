...
export async function auditToolUsage(sessionsRoot: string): Promise<UsageAudit> {
  ...
  if (zstdUnsupported > 0) notes.push(`${zstdUnsupported} zstd-compressed session log${zstdUnsupported === 1 ? '' : 's'} skipped: this Node runtime lacks node:zlib zstd support (Node ≥22.15 enables usage data)`)
