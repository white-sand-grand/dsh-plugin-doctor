...
`plugin_landscape` joins three sources: installed inventory (including each installed package manifest's description, keywords, and dependencies), the usage audit above, and — when an intent is given — community candidates. Tiers combine usage volume/recency with irreplaceability from the same similarity analysis that powers dedupe: `core` (≥10 calls, or used with irreplaceability ≥0.7), `active` (some calls), `idle` (declared tools, zero calls), `review` (idle plus stale-or-redundant), `unattributed` (no measurable usage). The relation graph renders as styled Mermaid (`graph LR`, edges capped strongest-first at 15): nodes carry Chinese function summaries, while edges and the following detail list identify text, capability, and dependency overlap. A system-prompt rule directs ordinary “how are my plugins related?” requests to this graph. All rendering logic lives in `landscape.ts` as pure functions.
